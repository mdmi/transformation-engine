...
public class GYearMonthConverter implements IConvertFromString {
	private static final String PATTERN_STRING = "(-?[0-9]{4}-[0-9]{2})((Z|[\\+-][0-9]{2}:[0-9]{2})?)";

	private static final Pattern s_pattern = Pattern.compile(PATTERN_STRING);

	private static final String s_defaultDay = "01";

	private static final String s_defaultTimezone = "Z";

	private static final IConvertFromString s_converter = new YearMonthDayConverter();
	...
	public Object convertFromString(String value, String format) {
		...
		if (match.matches()) {
			...
			builder.append(match.group(1)).append("-").append(s_defaultDay).append(timezone);
			return s_converter.convertFromString(builder.toString(), format);
		} else {
			// Did not match pattern => not valid.
			throw new IllegalArgumentException("Unable to parse '" + value + "' as a gYearMonth.");
		}
	}
}
